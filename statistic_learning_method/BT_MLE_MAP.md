...
- 贝叶斯公式中$P(\theta | X) =  \frac{P(X | \theta) \times P(\theta)}{P(X)}$ 在最大后验估计和最大似然中有一个基本的假设是认为当前发生的事件概率最大， 通过带入具体的 $X=x_0$ 来求解参数。因此可以不用考虑 分母， 因为当$X=x0$ 的时候分母就是一个常数了。 但是现在我们希望求的是 $P(\theta | X)$ 这个分布， 因此 $P(X)$ 是不能够当成常数处理的， 这是一个分布。 $P(X)$ 可以通过联合概率进行求解：$\int_{\theta} P(X | \theta) d\theta$ . 
